...
window.ShearPhoto.MINGGE(function() {
		          //██████████重要设置████████████████
	               ...
	               var relativeUrl= ""; //shearphoto_common你不要在后面加斜杠，系统会自动给你加上斜杠，不信看下面！   index.html的JS引用路径自己改，很简单的说
	 ...
	 */
		...
		/*HTML5重点功能*/	 HTML5ZIP:[900,0.9],//HTML5截图前载入的大图 是否压缩图片(数组成员 是数字) ，如果不压缩的话填false，在处理特效时或者拉伸时会明显出卡顿,不流畅！官方强烈建意你进行设置 ，默认填写的是[800,0.9] ,代表宽和高都不能大于800，质量是0.9（最大是1） 
			   ...
			   /*记住 preview 尽量设false*/ 
					          ...
					          url:relativeUrl+"compare/singleUpload",   //后端处理地址，保证正确哦，这是常识，连这个地址都能写错，你就是菜B，已经在本版本中帮你加入相对路径，你基本不用改这里了
			        
					    scopeWidth:500,                 //可拖动范围宽  也就是"main"对象的初始大小(整数型，禁止含小数点)  
                    
					   scopeHeight:400,                //可拖动范围高  也就是"main"对象的初始大小(整数型，禁止含小数点)    
                    
				      proportional:[0,              // <!--截框的宽高比例（宽除以高的比例值，这个设置其实就是1，对！你可以直接写1  如填3/4 那么就是0.75的比例,不设比例请设为0，注意更改比例后，后端也要进行相应设置，否则系统会给你抱出错误-->
					 
					 500,                      //必须整数！启动后的截框初始宽度(整数型，禁止含小数点)  
					
					 400                       //比例设置后，这个高度无效，由宽和比例来决定(整数型，禁止含小数点)  
					  ],   
...
var up = new ShearPhoto.frameUpImg({
	         ...
	         url:relativeUrl+"compare/singleUpload",            //HTML5切图时，不会用到该文件，后端处理地址，保证正确哦，这是常识，连这个地址都能写错，你就是菜B，已经在本版本中帮你加入相对路径，你基本不用改这里了
...
//	  alert(document.getElementById("ShearPhotoForm").getAttribute('enctype'));
//	  document.getElementById("ShearPhotoForm").submit();
});
